...
def get_peaks_sources(peaks_df: List[pd.DataFrame], test_peak: List[str]=[])->Tuple[List[str],List[str],dict[str, str]]:
    """
        ...
        -------
        List[str]
            Gene of the wt dataset.
        List[str]
            Genes of the training dataset
        dict[str, str]
            A mapping from gene to id. 
    """

    gene_to_id = assign_id_to_gene(peaks_df)
    wt_df = [peaks_df[-1]["gene"].iloc[0]]
    multiple_df = [df["gene"].iloc[0] for df in peaks_df[:-1] if df["gene"].iloc[0] not in test_peak]
    
    return wt_df, multiple_df, gene_to_id
...
def run_training(peaks_df: list[pd.DataFrame], train_peaks: List[str], test_peaks: Optional[List[str]], cfg: Config, 
    ...
    for j in range(cfg.train_cfg.num_runs):
        ...
        exp_name:str = get_experiment_name(cfg, train_peaks, test_peaks, round=j) + f"_{split}"

        train_df_merged, test_df_merged = separate_peaks(peaks_df, train_peaks, test_peaks)
        
        train_df, val_df, unlabeled_df = split_data(train_df=train_df_merged, unlabeled_df=test_df_merged, data_cfg=cfg.data_cfg, split=split)
